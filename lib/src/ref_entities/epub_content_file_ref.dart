...
abstract class EpubContentFileRef {
  ...
  ArchiveFile getContentFileEntry() {
    ...
    var archive = epubBookRef.EpubArchive()!;
    var contentFileEntry = archive.files.firstWhereOrNull(
      (ArchiveFile x) => x.name == contentFilePath,
    );

    if (contentFileEntry == null) {
      // 👇 Fallback: ลอง decode ทั้งสองข้างแล้วเทียบ
      final fallbackPath = _tryDecode(contentFilePath ?? '');
      contentFileEntry = archive.files.firstWhereOrNull(
        (x) => _tryDecode(x.name) == fallbackPath,
      );
    }
  ...
  String _tryDecode(String? input) {
